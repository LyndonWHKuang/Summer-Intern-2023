...
def get_utility_metrics(real_path, fake_paths, scaler="MinMax", classifiers=["lr", "dt", "rf", "mlp"], test_ratio=.20,
    ...
    X_train_real, X_test_real, y_train_real, y_test_real = model_selection.train_test_split(data_real_X, data_real_y,
    ...
    for classifier in classifiers:
        real_results = supervised_model_training(X_train_real_scaled, y_train_real, X_test_real_scaled, y_test_real,
                                                 classifier)
    ...
    for fake_path in fake_paths:
        ...
        # Splitting synthetic data to obtain corresponding synthetic training dataset
        data_fake_y = data_fake[:, -1]
        data_fake_X = data_fake[:, :data_dim - 1]
        X_train_fake, _, y_train_fake, _ = model_selection.train_test_split(data_fake_X, data_fake_y,
                                                                            test_size=test_ratio, stratify=data_fake_y,
                                                                            random_state=42)
        ...
        all_fake_results = []
        for classifier in classifiers:
            fake_results = supervised_model_training(X_train_fake_scaled, y_train_fake, X_test_real_scaled, y_test_real,
                                                     classifier)
...
def stat_sim(real_path, fake_path, cat_cols=None):
    ...
    for column in real.columns:
        ...
        if column in cat_cols:
            ...
            # Computing the real and synthetic probability mass distributions (pmf) for each categorical column
